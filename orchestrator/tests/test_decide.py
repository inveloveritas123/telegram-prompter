...
class TestContinue:
    def test_gates_nok_kein_promise(self):
        action, reason = decide(gates_ok=False, promise=False, iteration=1, max_iter=10)
        assert action == "continue"
        assert "weiterarbeiten" in reason

    def test_gates_ok_kein_promise(self):
        action, reason = decide(gates_ok=True, promise=False, iteration=1, max_iter=10)
        assert action == "continue"
        assert "promise fehlt" in reason

    def test_gates_nok_promise(self):
        action, reason = decide(gates_ok=False, promise=True, iteration=1, max_iter=10)
        assert action == "continue"
...
class TestStop:
    def test_gruen_und_promise(self):
        action, reason = decide(gates_ok=True, promise=True, iteration=1, max_iter=10)
        assert action == "stop"
        assert "GRUEN" in reason

    def test_gruen_und_promise_erste_iteration(self):
        action, reason = decide(gates_ok=True, promise=True, iteration=0, max_iter=10)
        assert action == "stop"
